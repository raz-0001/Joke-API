...
app.get("/filter",(req,res)=>{
  ...
  const jokec=jokes.filter((j)=>j.jokeType===type);
  res.json(jokec);
})
...
app.delete("/jokes/:id",(req,res)=>{
  ...
  if(existingIndex>-1){
    jokes.splice(existingIndex,1);
    res.status(200);
    res.json({message:`Successful!`});
  }
  else{
    res.status(404);
    res.json({message:`No such joke with index ${id} exists`});
  }
...
app.delete("/all",(req,res)=>{
  ...
  if(userkey==masterKey){
    ...
    res.status(404);
    res.json({error:"You are not authorised to perform this."});
  }
})
